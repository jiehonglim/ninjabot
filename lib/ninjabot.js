...
//        console.log('url is ', self.splunk_url);
        ...
        payload.message.channel_id = message.channel
        payload.message.channel_name = channel.name
        payload.message.user = message.user;
        payload.message.display_name = self.convertToUserName(message.user);
...
NinjaBot.prototype._isFromNinjaBot = function (message) {
    ...
    console.log ('this id is ', id, 'this name is ', this.name);
...
//  console.log('array nm', userid , 'userid name is', this.users[userid].name, 'userid is', this.users[userid].id);

    if( userid == key || this.users[userid].id == key ){
